 ...
 * 定义数据源的AOP切面，通过该service.dao的方法名判断是应该走读库还是写库
 */
//@Component
//@Aspect
...
//        }
    ...
    public void before(JoinPoint point) {
        ...
        if (isSlave) {
            // 标记为读库
            DynamicDataSourceKeyHolder.markSlave();
        } else {
            // 标记为写库
            DynamicDataSourceKeyHolder.markMaster();
        }
    }
    ...
    public String[] getSlaveMethodStart() {
        return this.slaveMethodStart == null ? defaultSlaveMethodStart : slaveMethodStart;
    }
    ...
    private Boolean isSlave(String methodName) {
        ...
        return StringUtils.startsWithIgnoreCase(methodName, "query")
                || StringUtils.startsWithIgnoreCase(methodName, "find")
                || StringUtils.startsWithIgnoreCase(methodName, "get");
    }
}
